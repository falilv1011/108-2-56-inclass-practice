...
情境:
```{r}
sum(list_victimAges_female$`數目`, na.rm=T)  #將na以外的值加總起來
...
# 創造10歲間距的所有可能   # 序列（Sequences）
start <- seq(5, 55, by=10) # 由5到55，每個值差10的數值向量  # seq 的步長可以使用 by 參數指定
end <- seq(14, 64, by=10)
middleLevels <- 
  rep(
    paste0(start,"-",end), 
    each=2) # 每個新間距要2個  # rep( [ 指定的向量], [使之重複的次數])
...
```{r}
library(lubridate)
dateInput <- today()  #today是一個lubridate的函數
locationInput <- "新北市"
```
...
```{r}
# 先執行此行輸入學號
readline(prompt = "請輸入你的學號") ->
  studentId 

# 之後才執行以下程式
# condition : TRUE 學號不正確
library(stringr)
if(
...
2.`check_package()`一次只能偵測一個套件是否有安裝。寫一個`check_packages(pkgNames)` (packages是複數)，它可以給pkgNames input, pkgNames為class character, length任意。
```{r}
# 測試
...
3.將`convert_TaiwanDate2WesternDate`的body有關套件檢查的部份改成你設計的check_package/check_packages。
...
* 創造一個`.R_Cache環境`並把weather_next36hours移到那裡放。  暫存的概念
...
各情境一定要互斥，即不可以有兩個情境有可能同時為TRUE，
如果發生多個情境吻合會以第一個結果的body來執行。
...
依照上一個特性，`function(...){....}`的body若只有一行指令可省略{...}改寫成`function(...) ...`, 只是要小心body … 要接在function(...)之後不可換行。
```{r}
miniFun <- function(){
  lubridate::now()
}
...
依照上一個特性，function(...){....}的body若最後一個「被執行」的指令是會印在螢幕的值，如該值要當回傳值可以不用`return(...)`就可以產生值回傳效果。
...
可適當使用`cut()`函數將與「連續型」數值變數有關的condition轉成需要的字串表示，
```{r}
cut(x, c(a,b,c))
```
 ...
 * x只要type是`integer`, `double`的資料都可使用，所以日期，時間都可以
...
```{r}
grade <- sample(1:100, 10, replace=T) # 1-100 隨機抽出10個
grade

grade %>% # 使用pipe
  cut(c(-1,69,79,89,100)) -> grade # 回存到grade,值不是分數了,變成類比資料

levels(grade)

switch(
  as.character(grade[[1]]),  # switch要用字串
  "(-1,69]"={
    print("-_-")
    "F"},
  "(69,79]"={
 ...
 * 晚上(17:00-19:00)： 媽媽準備全家晚餐點foodpanda，爸爸瀏覽tripadvisor(https://www.tripadvisor.com/)規
   劃週末全家旅行，提醒小明“作業寫好了嗎？”

 * 晚上(23:30-次日淩晨1點)： 媽媽聆聽Spotify Jazz  
...
綜合練習:
1.上面的流程其實也可以用if… else if… else來做而不用switch，請用if… else if… else來創造whatNeedToDo_butler_mom。
```{r}

```

2.請完成給爸爸和小明的管家機器人，whatNeedToDo_butler_dad和whatNeedToDo_butler_ming。
```{r}
...
由於`if`, `if…else`, `if…if else…else`, 都是用來決定要不要執行某個body，所以(condition) 必需要是「一個」T/F logical value。
...
函數使用時若出現output value assigned動作時（即pass_fail_i <-），
相當於執行了函數body pass_fail_i <-{...}，依switch小節的{...}處理原則：
